...
class Parser {
  ...
  definition() {
    if (!this.compareType("^(TEXT)+(SPACE)*(EQUAL_SIGN)$")) {
      return;
    }
  ...
  alpha() {
    ...
    // Expanded node
    const ast = convertToAST("UPALPHA | LOALPHA");
    this.ast.definitions["ALPHA"] = ast.children;
  }
  ...
  crlf() {
    ...
    // Expanded node
    const ast = convertToAST("CR LF");
    this.ast.definitions["CRLF"] = ast.children;
  }
  ...
  lws() {
    ...
    // Expanded node
    const ast = convertToAST("[CRLF] 1*( SP | HT )");
    this.ast.definitions["LWS"] = ast.children;
  }
  ...
  hex() {
    ...
    // Expanded node
    const ast = convertToAST('"A" | "B" | "C" | "D" | "E" | "F" | "a" | "b" | "c" | "d" | "e" | "f" | DIGIT');
    this.ast.definitions["HEX"] = ast.children;
  }
  ...
  tspecials() {
    ...
    // Expanded node
    const ast = convertToAST('"(" | ")" | "<" | ">" | "@" | "," | ";" | ":" | "\" | <"> | "/" | "[" | "]" | "?" | "=" | "{" | "}" | SP | HT');
    this.ast.definitions["tspecials"] = ast.children;
  }
